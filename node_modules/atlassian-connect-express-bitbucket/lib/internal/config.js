...
module.exports = function (mode, overrides) {
    var addOnDir = env['AC_ADDON_DIR'] || process.cwd();

    var config = utils.loadJSON(path.join(addOnDir, 'config.json'));
    var publicKey = utils.loadFile(path.join(addOnDir, 'public-key.pem'));
    var privateKey = utils.loadFile(path.join(addOnDir, 'private-key.pem'));

    config = _.extend(config, overrides, {
      addOnDir: addOnDir,
      descriptorFile: path.join(addOnDir, 'atlassian-connect.json')
    });
    ...
    return _.extend({}, wrap(globalValues), wrap(modeValues), {
        ...
        bitbucket: function() {
            return get(modeValues, 'bitbucket', 'AC_BITBUCKET');
        },

        addonHome: function () {
            return get(modeValues, 'addonHome', 'AC_ADDON_HOME');
        },

        consumerKey: function () {
            return get(modeValues, 'consumerKey', 'AC_CONSUMER_KEY');
        },
        ...
        descriptorFile: function() {
            return get(globalValues, 'descriptorFile');
        },

        addOnDir: function() {
            return get(globalValues, 'addOnDir');
        },
